...
| --- | --- | --- | --- | --- | --- | --- |
| BAD-002 | P0 | REST/API security | Scoped credentials, actor-bound writes, and API write receipts now cover current high-authority API families, including operations planning/cutover, calls/transcripts, worker-time review, Kent tender/config writes, labor absence, and importer writes. Credential rotation/deprecation docs and exhaustive denial/receipt coverage are not complete. | `corkysoft/api_shared.py` provides `ApiAuthContext`; `corkysoft/api_operations.py`, `api_calls.py`, `api_kent.py`, `api_labor.py`, and `api.py` migrate current high-authority families with scoped-write tests in `tests/test_api.py`. | Security/API governance | Finish rotation/deprecation docs and add denial/receipt tests for every write family. |
...
| BAD-005 | P1 | Dashboard operations | Dispatch can crash after filters remove all rows because selection code indexes an empty options map. | `dashboard/components/dispatch.py`. | Dashboard shell | Empty-state guard before selectbox/indexing; test with zero filtered rows. |
