...
| BAD-003 | P0 | Transcript/audio boundary | Corrected for promotion governance. Advisory transcript evidence can only create a held `evidence_promotions` proposal; a scoped `evidence:review` actor may accept, reject, or hold it with durable decision history and API receipt. Promotion records never themselves apply notes, worker time, job state, or customer projections. | `corkysoft/evidence_promotion.py`, `corkysoft/api_calls.py`, `tests/test_call_ops.py`, `tests/test_api.py`. | Security/API governance | Preserve proposal-only behavior for new model/browser/PNF evidence sources and route any future effect through a separately authorised write. |
