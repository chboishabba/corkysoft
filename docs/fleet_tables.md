...
Driver shifts are stored in `driver_shifts`, keyed by shift date and optionally linked to `trucks` and `workers`. The table records ticket numbers, shift start/end, hours, hourly rate, total cost, free-form notes, and the source sheet name used for import; a uniqueness constraint across date/truck/worker/time/tickets guards against accidental duplicates.【F:analytics/db.py†L177-L195】 The `analytics.driver_shifts` module imports the `VEHICLE_DRIVER` worksheet, resolves aliases for driver, vehicle, and time fields, backfills `cost_total` when only hours and rate are present, and writes through to the shared tables via `upsert_driver_shift`.【F:analytics/driver_shifts.py†L1-L176】 Queries that join worker and truck names for auditing live in `fetch_driver_shifts`, enabling downstream dashboards to filter by date range, worker, or vehicle while preserving the imported metadata.【F:analytics/db.py†L788-L825】
